...
C      END
      ...
      REAL FUNCTION RADMOTT1(THETX)
C
C===     Radiative Mott scattering
C        Levchuk's code, slightly adapted
C
      REAL  THETX   ! - theta angle (rad) (or a function of theta)
     +     ,ELOSS   ! - the fraction of the beam energy, left to the electron (radiative losses)
C
C       COMMON/CCSMOTT/ CSDMOTT(4),ALMOTT
      REAL            CSDMOTT,ALMOTT
...
C
      tanang=TAN(thet)
C         TANANG=SQRT(2.*WE/(E0+WE))
      CTT=1./SQRT(1.+TANANG**2)
C         E1=0.5*(E0+WE)
      E1=E0*ELOSS
      CALL RTAIL(E0,E1,CTT,ZAT,AAT,NPOINT,SIG)
***
C     GET THE CROSS SECTION IN 'barn/ster/GeV'
***
      SIGMA=10.0*SIG/FMMEV
      RADMOTT1=SIGMA*SIN(thet)
      IF(ABS(ALMOTT).GT.1.E-6) THEN
         RADMOTT1=RADMOTT1/THETX/ALMOTT
      ENDIF
C      WRITE(6,1000) E0,eloss,thet,SIGMA
 1000    FORMAT(5X,'E0 =',E13.5,'  GeV',4X,'E left=',E10.3
     +    ,5X,'theta =',E11.4
     +    ,5X,'SIGMA =', E11.4,'  barn/ster/GeV')
 100  CONTINUE
      END
C
      SUBROUTINE RTAIL(E0,E1,CTT,ZAT,AAT,NPOINT,SIGMA)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      PARAMETER (PI=3.141592654D0)
      COMMON /KGAU/ KTBAX(97)
      COMMON /GAUSS/ UX(273),VX(273)
      COMMON /CONSTM/WE,WE2,WFE,WFE2
      COMMON /HELP/SMOD,PMOD,UMOD,SSPP,ST,CTS,STS,CTP,STP
      DOUBLE PRECISION S(3),P(3),U(3)
      DIMENSION A(2),B(2)
      DIMENSION F(0:10000)
      REAL E0,E1,CTT,ZAT,AAT,SIGMA,UX,VX
*
      ALPHA=1.D0/137.03602D0
      FMGEV=.197327053D0
      WE=0.511D-3/FMGEV
      WE2=WE*WE
      WNP=.93827231D0/FMGEV
      WNN=.93956563D0/FMGEV
      ZFE=ZAT
      AFE=AAT
      WFE=ZFE*WNP+(AFE-ZFE)*WNN-AFE*7.D-3/FMGEV
      WFE2=WFE*WFE
      ES=E0/FMGEV
      EP=E1/FMGEV
      CT=CTT
      SMOD=DSQRT(ES*ES-WE2)
      PMOD=DSQRT(EP*EP-WE2)
C
      S(1)=0.D0
      S(2)=0.D0
      S(3)=SMOD
      ...
      DO 10 J=1,3
         U(J)=S(J)-P(J)
   10 CONTINUE
      UMOD=DSQRT(DSCAL(U,U))
      SSPP=ES*EP-SMOD*PMOD*CT
      CTS=DSCAL(S,U)/SMOD/UMOD
      CTP=DSCAL(P,U)/PMOD/UMOD
      STS=DSQRT(1.D0-CTS*CTS)
      STP=DSQRT(1.D0-CTP*CTP)
*
      ANORM=ZFE**2*(ALPHA**3/(2.D0*PI)**2)*(EP/ES)/WFE
*
      CTKPOL=( (ES/SMOD)*STP-(EP/PMOD)*STS )/ST
 3000 format(3x,'ctkpol =',e13.5)
       IF(CTKPOL.GT.-1.D0.AND.CTKPOL.LT.1.D0) THEN
        NINT=2
****        A(1)= -1.D0
****        B(1)= CTKPOL - 1.D-8
****        A(2)= CTKPOL + 1.D-8
****        B(2)= 1.D0
        A(1)= 0.D0
        B(1)= DACOS(CTKPOL) - 1.D-8
        A(2)= DACOS(CTKPOL) + 1.D-8
        B(2)= PI
       ELSE
        NINT=1
****        A(1)=-1.D0
...
*
         DO 100 J=1,NP1
            JJ=J-1
****           CTK=B(I)-(NPOINT-JJ)*DCTK
            ANG=B(I)-(NPOINT-JJ)*DANG
            CTK=DCOS(ANG)
            F(JJ)=TSAIMO(ES,S,EP,P,CT,U,CTK)
 900        format(10x,'f =',e13.5)
 100     CONTINUE
C         WRITE(6,FMT='(10D12.3)') (F(j),j=0,NPOINT)
C         WRITE(6,FMT='(I5,10D12.3)') I,A
C         WRITE(6,FMT='(I5,10D12.3)') I,B
         DO j=0,NPOINT
            IF(F(j).LT.0.D0) THEN
               WRITE(6,*) j,F(j)
C               F(j)=0.D0
            ENDIF
         ENDDO
         RR=RR+SIMPS(F,A(I),B(I),NPOINT)
C         WRITE(6,*) 'RR=',RR
 200  CONTINUE
C
C!!!! note the 'nonstandard' normalisation for the form factor
C!!!! in the paper by L.W.Mo and Y.S.Tsai (see Eqs. (III.2)-(III.4)) 
C!!!! the extra factor 4.0 below results from the conv. of this paper
C
      SIGMA=4.0D0*ANORM*RR
      END
C
      DOUBLE PRECISION FUNCTION TSAIMO(ES,S,EP,P,CT,U,CTK)
C     all the momenta here are in fm**(-1)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      PARAMETER (PI=3.141592654D0)
      COMMON /CONSTM/WE,WE2,WFE,WFE2
      COMMON /HELP/SMOD,PMOD,UMOD,SSPP,ST,CTS,STS,CTP,STP
      DIMENSION S(3),P(3),U(3)
      INTEGER ipr
      SAVE ipr
      DATA ipr/0/
C
      PU0=ES+WFE-EP
**      W=.5D0*(PU0**2-UMOD**2-WFE2)/(PU0-UMOD*CTK)
      W=(ES-EP-(SSPP-WE2)/WFE)/(1.D0+(ES-EP-UMOD*CTK)/WFE)
      Q2=2.D0*(WE2-SSPP-W*(ES-EP)+W*UMOD*CTK)
      FORFAC=FFAC(Q2)
      STK=DSQRT(1.D0-CTK*CTK)
      A=W*(EP-PMOD*CTP*CTK)
      B=-W*PMOD*STP*STK
      A1=W*(ES-SMOD*CTS*CTK)
      B1=-W*SMOD*STS*STK
      DENNU=W*(EP*SMOD*STS-ES*PMOD*STP+SMOD*PMOD*ST*CTK)
      XNU =-PMOD*STP/DENNU
      XNU1=-SMOD*STS/DENNU
      DENFAS=Q2**2 * (PU0-UMOD*CTK)
      FASFAC=.5D0*W/DENFAS
      FACTE=WFE2*FORFAC**2
*
      DENAB =DSQRT(A **2-B **2)
      DENAB1=DSQRT(A1**2-B1**2)
*
      X1=-(2.D0*PI*A *WE2/(DENAB **3))*(2.D0*ES*(EP+W)+.5D0*Q2)
      X2=-(2.D0*PI*A1*WE2/(DENAB1**3))*(2.D0*EP*(ES-W)+.5D0*Q2)
      X3=-4.D0*PI
      X4=-X3*(XNU/DENAB-XNU1/DENAB1)*
     *   ( WE2*(SSPP-W**2) + SSPP*(2.D0*ES*EP-SSPP+W*(ES-EP)) )
      X5= (2.D0*PI/DENAB )*( 2.D0*(ES*EP+ES*W+EP**2)+.5D0*Q2-SSPP-WE2 )
      X6=-(2.D0*PI/DENAB1)*( 2.D0*(ES*EP-EP*W+ES**2)+.5D0*Q2-SSPP-WE2 )
*
      TSAIMO=FASFAC*FACTE*(X1+X2+X3+X4+X5+X6)*STK
      xx=x1+x2+x3+x4+x5+x6
      IF(TSAIMO.LT.0.D0.OR.ipr.GT.0) THEN
         IF(TSAIMO.LT.0.D0) THEN
            ipr=1
         ELSE
            ipr=0
         ENDIF
C         WRITE(6,FMT='(1X,A3,10D12.4)') 'PAR',ES,EP,CT,CTK
C         WRITE(6,FMT='(1X,A3,10D12.4)') '   ', 
C     +         FASFAC,FACTE,X1,X2,X3,X4,X5,X6,STK,xx
C         WRITE(6,FMT='(1X,A3,10D12.4)') '   ', 
C     +         DENAB,DENAB1,ES,EP,W,Q2,SSPP,WE2,XNU,XNU1
      ENDIF
      END
C
      DOUBLE PRECISION FUNCTION FFAC(QQ2)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C     COMPUTES THE CHARGE FORM FACTOR OF 12C NUCLEUS
      FFAC=1.D0
*******
      return
*******
      Q2=-QQ2
       IF(Q2.GT.3.2D0.AND.Q2.LE.3.5D0) THEN
       FFAC=1.0D-5
       RETURN
       ENDIF
      Z=6.0D0
      ALFA=(Z-2.0D0)/3.0D0
      IF(Q2.LE.3.2D0) A=1.64D0
      IF(Q2.GT.3.5D0) A=1.68D0
      ARG=Q2*A*A/4.D0
      F=1.D0 - 2.0D0*ALFA*ARG/(2.D0+3.D0*ALFA)
      FFAC=F*DEXP(-ARG)
      END
C
      FUNCTION SGMOTT(E,TETA)
C     COMPUTES THE MOTT CROSS SECTION
C     TETA- IN RADIANS, E- IN FM**(-1)
      A=137.03602
      ALPHA=1./A
      HALFT=0.5*TETA 
      SINTET=SIN(HALFT)
      F=ALPHA*COS(HALFT)/(2.*E*SINTET*SINTET)
      SGMOTT=F*F
      END
C
C
C ------------------FORM FACTORS----------------------------- 
C
      FUNCTION FF(NUM,CHAR,Q2FM)
C     THE FORM-FACTOR OF A NUCLEON ACORDING TO
C     J.G.KORNER AND M.KURODA // PHYS.REV. D16, P.2165 (1977)
C     Q2 - IN (FM)**(-2)
      PARAMETER (ALPHA=0.95)
      PARAMETER (FMGEV2=.038937966)
      CHARACTER*1 CHAR
      DATA AKP /1.7928456/, AMP /2.7928456/, AMN /-1.91304184/
*
      A=ALPHA/FMGEV2
*
      IF(NUM.EQ.1 .AND. CHAR.EQ.'V') THEN 
         NPOLE=2
         W2=.593/FMGEV2
         C=1.
      ENDIF         
*
      IF(NUM.EQ.1 .AND. CHAR.EQ.'S') THEN 
         NPOLE=2
         W2=.614/FMGEV2
         C=1.
      ENDIF         
*
      IF(NUM.EQ.2 .AND. CHAR.EQ.'V') THEN 
         NPOLE=3
         W2=.593/FMGEV2
*         C=(5./6.)*AMP-(5./4.)*AMN-1.
         C=AKP-AMN
      ENDIF         
*
      IF(NUM.EQ.2 .AND. CHAR.EQ.'S') THEN 
         NPOLE=3
         W2=.614/FMGEV2
*         C=(1./6.)*AMP-(1./4.)*AMN-1.
         C=AKP+AMN
...
C
C
      DOUBLE PRECISION FUNCTION DSCAL(A,B)
C     SCALAR PRODUCT
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DIMENSION A(3),B(3)
      DSCAL=A(1)*B(1)+A(2)*B(2)+A(3)*B(3)
      RETURN
      END
      SUBROUTINE VECT(A,B,ABV)
C     VECTOR PRODUCT
      DIMENSION A(3),B(3),ABV(3)
      ABV(1)=A(2)*B(3)-A(3)*B(2)
      ABV(2)=A(3)*B(1)-A(1)*B(3)
      ABV(3)=A(1)*B(2)-A(2)*B(1)
      RETURN
      END
C
C
      DOUBLE PRECISION FUNCTION SIMPS(F,A,B,N2)
C     SIMPSON INTEGRATION /CERNLIB,D101/
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      CHARACTER NAME*(*)
      CHARACTER*80 ERRTXT
...
*
    ...
    2 CONTINUE
      END
C
      DOUBLE PRECISION FUNCTION GAUSUM(F,A,B,N)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      PARAMETER (HF=0.5)
      COMMON /KGAU/ KTBA(97)
      COMMON /GAUSS/ U(273),V(273)
      REAL U,V
      DIMENSION F(N)
      ALFA=HF*(B+A)
      BETA=HF*(B-A)
